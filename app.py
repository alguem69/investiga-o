...
def render_classified_header() -> None:
    """Cabecalho principal limpo e minimalista."""
    st.markdown(
        f"""
...
def render_tab_nova() -> None:
    ...
    st.caption("Use o chat para notas rapidas e o formulario para registrar o caso.")
...
def render_tab_config() -> None:
    st.caption("Gerenciamento local dos dados e informacoes operacionais.")
...
def main() -> None:
    ...
    tab1, tab2, tab3 = st.tabs(["Nova Investigacao", "Historico", "Configuracoes"])
    with tab1:
        render_tab_nova()
    with tab2:
